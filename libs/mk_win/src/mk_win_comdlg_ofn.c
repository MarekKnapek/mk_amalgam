...
mk_jumbo int mk_win_comdlg_ofn(mk_win_comdlg_ofn_t* ofn, mk_win_base_types_bool_t* ret)
{
	...
	mk_try(mk_win_str_from_api_z(ofnr.m_custom_filter, 1, &ofn->m_custom_filter));
	mk_try(mk_win_str_from_api_s(ofnr.m_file, ofn->m_file_name_buff_len_chars, 2, &ofn->m_file_name_out));
	mk_try(mk_win_str_from_api_z(ofnr.m_file_title, 3, &ofn->m_file_title));
	mk_assert(ret);
	*ret = b;
	return 0;
	#else
	...
	if(!unicode_enabled)
	{
		...
		mk_try(mk_win_str_from_narrow_z(ofnr.m_custom_filter, 1, &ofn->m_custom_filter));
		mk_try(mk_win_str_from_narrow_s(ofnr.m_file, ofn->m_file_name_buff_len_chars, 2, &ofn->m_file_name_out));
		mk_try(mk_win_str_from_narrow_z(ofnr.m_file_title, 3, &ofn->m_file_title));
		mk_assert(ret);
		*ret = b;
		return 0;
	}
	else
	{
		...
		mk_try(mk_win_str_from_wide_z(ofnr.m_custom_filter, 1, &ofn->m_custom_filter));
		mk_try(mk_win_str_from_wide_s(ofnr.m_file, ofn->m_file_name_buff_len_chars, 2, &ofn->m_file_name_out));
		mk_try(mk_win_str_from_wide_z(ofnr.m_file_title, 3, &ofn->m_file_title));
		mk_assert(ret);
		*ret = b;
		return 0;
	}
